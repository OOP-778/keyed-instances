...
public interface KeyedInstances {
     ...
     *
     * @param root     the root class of the instance tree, can be null to search from the top level
     * @param fullPath the full path of the instance
     * @return a {@link KeyedInstance} associated with the given full path, or null if no instance found
     */
    @Nullable
    KeyedInstance getInstance(@Nullable Class<? extends KeyedInstance> root, @NonNull String fullPath);
     ...
     */
    default Optional<KeyedInstance> getOptionalInstance(@Nullable Class<? extends KeyedInstance> root, String path) {
        return Optional.ofNullable(this.getInstance(root, path));
    }
    ...
    void walk(@NonNull KeyedWalker walker);
}
